        ...
        onSuccess: async (appointment: any) => {
          ...
          try {
            ...
            try {
              ...
              console.log("Email API response text length:", responseText?.length || 0);
              ...
              if (responseText && responseText.trim()) {
                try {
                  ...
                  console.log("Email API response is not JSON, keeping as text");
                  console.log("Email API response text preview:", responseText.substring(0, 200));
                }
              } else {
            ...
            if (!emailResponse.ok) {
              ...
              console.warn("⚠️ Email notification failed (appointment was still booked successfully)");
              console.warn("Status:", status);
              console.warn("Message:", message);
              
              // Only log detailed error info if it's not the common "API key missing" error
              if (message && !message.includes("RESEND_API_KEY") && !message.includes("not configured")) {
                if (responseText && responseText.trim()) {
                  console.warn("Response Text:", responseText.substring(0, 300));
                }
                if (responseData && Object.keys(responseData).length > 0) {
                  console.warn("Response Data:", responseData);
                }
              } else {
                console.info("💡 To enable email notifications, add RESEND_API_KEY to your .env.local file");
              }
              ...
              console.log("✅ Email sent successfully:", responseData || "Response OK");
            }
          } catch (error) {
      ...
      {userAppointments.length > 0 && (
        <div className="mb-8 max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {userAppointments
              .filter((appointment: any) => appointment.doctor || appointment.doctorName)
              .map((appointment: any) => {
              const doctorName = appointment.doctor?.name || appointment.doctorName || "Unknown Doctor";
              const doctorImageUrl = appointment.doctor?.imageUrl || appointment.doctorImageUrl || "";
              
              // Debug log (remove in production)
              if (!appointment.doctor && !appointment.doctorName) {
                console.warn("Appointment missing doctor data:", appointment.id);
              }
